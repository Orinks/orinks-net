...
async function uploadReviewAware(
  t: ReturnType<typeof setup>,
  auth: { driverId: string; driverTokenHash: string },
...
describe("validated private cloud revisions", () => {
  ...
  test("the digest lists waiting careers only when something is new", async () => {
    ...
    expect(first).toMatchObject([{ saveName: "Road Star", isNew: true, observations: 1 }]);
    expect(first[0].content).not.toBeNull();
    await t.mutation(internal.freightFateReview.markDigestSent, {
      ids: first.map((row) => row.id), now: first[0].lastObservedAt,
    });
    // Nothing new: no email, even though the review still waits.
    expect(await digest()).toEqual([]);
  ...
  test("course-only credentials are never derived from level", async () => {
    ...
    await expect(uploadMeaningfully(t, auth, payload, null, "credential-ladder"))
      .resolves.toMatchObject({ ok: true, revision: 1 });
    const snapshot = await t.run((ctx) => ctx.db.query("freightFateProfileSnapshots").first());
  ...
  test("backs up future catalog values without crashing or publishing invented facts", async () => {
    const t = setup();
    const auth = await provisionedDriver(t, "user_future_catalog");
    const payload = validProfile();
    Object.assign(payload, {
      business_status: "independent_authority",
      current_city: "future_city",
      truck: "future_tractor",
      owned_trucks: ["future_tractor"],
      owned_trailers: ["future_trailer"],
    });

    await expect(uploadMeaningfully(t, auth, payload, null, "future-catalog"))
      .resolves.toMatchObject({ ok: true, revision: 1 });
    const snapshot = await t.run((ctx) => ctx.db.query("freightFateProfileSnapshots").first());
    expect(snapshot).toMatchObject({
      sourceSaveName: "Road Star",
      netWorth: payload.money,
      netWorthComplete: true,
    });
    expect(snapshot).not.toHaveProperty("lastSavedCity");
    expect(snapshot).not.toHaveProperty("truckName");
  ...
  test("keeps the first verified slot as the public profile owner", async () => {
    const t = setup();
    const auth = await provisionedDriver(t);
    await uploadMeaningfully(t, auth, validProfile(), null, "owner-first");
    ...
    await uploadMeaningfully(t, auth, updatedOwner, 1, "owner-update");
    ...
    await expect(uploadMeaningfully(t, auth, experiment, null, "experiment-after-delete"))
      .resolves.toMatchObject({ ok: true, revision: 1 });
    expect(await t.run((ctx) => ctx.db.query("freightFateProfileSnapshots").first()))
  ...
  test("a designated public career owns the projection over the first uploader", async () => {
    const t = setup();
    const auth = await provisionedDriver(t);
    await uploadMeaningfully(t, auth, validProfile(), null, "designate-first");
    ...
    await expect(uploadMeaningfully(t, auth, second, 1, "designated-second"))
      .resolves.toMatchObject({ ok: true, revision: 2 });
    expect(await t.run((ctx) => ctx.db.query("freightFateProfileSnapshots").first()))
  ...
  test("re-designating the current public career keeps its projection", async () => {
    const t = setup();
    const auth = await provisionedDriver(t);
    await uploadMeaningfully(t, auth, validProfile(), null, "redesignate-current");
    await expect(t.mutation(api.freightFateSaves.setPublicSave, {
      ...auth, saveName: "Road Star", now: Date.now(),
  ...
  test("clearing the designation returns to the first-uploader rule", async () => {
    const t = setup();
    const auth = await provisionedDriver(t);
    await uploadMeaningfully(t, auth, validProfile(), null, "clear-first");
    ...
    await expect(uploadMeaningfully(t, auth, validProfile(), 1, "clear-reclaim"))
      .resolves.toMatchObject({ ok: true, revision: 2 });
    expect(await t.run((ctx) => ctx.db.query("freightFateProfileSnapshots").first()))
      .toMatchObject({ sourceSaveName: "Road Star" });
  });
